...
Page({
  ...
  data: {
    ...
    weekDayCount: 7,
    startDate: '2023/02/27', // 开学日期
    weekIndexText: ['一', '二', '三', '四', '五', '六', '日'], //中文映射
    nowMonth: 1, // 当前周的月份
    courseList: [],
    colorList: [
      "#116A7B",
   ...
   */
  onLoad(options) {
    const {windowWidth} = wx.getSystemInfoSync() //获得屏幕宽度用于定位周几课表
    this.setData({
      windowWidth
    })
    this.getWeekDates()
  ...
  navCourseDetail(e) {
    ...
    wx.navigateTo({
      url: `/pages/course-detail/course-detail?info=${JSON.stringify(this.data.courseList[index])}`,
    })
  }
})